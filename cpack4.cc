...
//DIFFERENZE: cambio condizioni al contorno, non metto piu' tutte uguali per ogni punto
...
void boundary_fun(double a[]) // mi pone le condizioni al bordo
{
  ...
  for(int i = 0; i < boundary; i++)
    {
      a[i] = 10 * drand48(); // scelgo raggio casuale fra 0 e 10
    }
}
  ...
  while(E > tolerance)
      ...
      for(j; j < interior; j++) // ciclo sui vertici interni
	{
	  double flower[interior][10] =
	    ...
	    }; // vettore (matrice) dei fiori, devo mettere qua perche' venga aggiornato ad ogni ciclo
